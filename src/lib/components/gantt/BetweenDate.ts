export class BetweenDate {
  ...
  constructor(start: Date, end: Date) {
    this.start = start;
    this.end = end;
    if (this.end <= this.start) { this.end = new Date(this.start.getDate() + 1); }
	}
  ...
  static MakeMonths(between: BetweenDate): BetweenDate[] {
		...
		let ret: BetweenDate[] = [];
		if (between.start.getFullYear() === between.end.getFullYear()
    ...
    while (i <= between.end) {
      ...
      if (tmpEnd > between.end) { tmpEnd = new Date(between.end); }
